...
import MaterialsModal from './components/MaterialsModal.tsx';
import GuideModal from './components/GuideModal.tsx';
import { Subject, Theme, Message, SubjectConfig, User } from './types.ts';
...
const App: React.FC = () => {
  ...
  const [theme, setTheme] = useState<Theme>(() => {
    return (localStorage.getItem('bk_theme') as Theme) || 'light';
  });
  ...
  const [subjectConfigs, setSubjectConfigs] = useState<Record<Subject, SubjectConfig>>(() => {
    ...
    if (saved) {
      ...
      Object.values(Subject).forEach(sub => {
        if (!parsed[sub]) parsed[sub] = { provider: 'gemini', modelId: 'gemini-3-pro-preview' };
      });
      return parsed;
    }
    const configs: any = {};
    Object.values(Subject).forEach(sub => {
      configs[sub] = { provider: 'gemini', modelId: 'gemini-3-pro-preview' };
    });
    return configs;
  });
  ...
  const [chatSessions, setChatSessions] = useState<Record<Subject, Message[]>>(() => {
    ...
    if (saved) {
      const parsed = JSON.parse(saved);
      Object.values(Subject).forEach(sub => {
        if (!parsed[sub]) parsed[sub] = [getWelcomeMessage(sub)];
      });
      return parsed;
    }
    const initial: any = {};
    Object.values(Subject).forEach((sub) => {
      initial[sub] = [getWelcomeMessage(sub)];
    });
    return initial;
  });
  ...
  useEffect(() => { localStorage.setItem('bk_admin_mode', JSON.stringify(isAdminMode)); }, [isAdminMode]);
  ...
  const handleClearHistory = (targetSubject?: Subject) => {
    ...
    setChatSessions(prev => ({
      ...prev,
      [subToClear]: [getWelcomeMessage(subToClear)]
    }));
  };

  const handleOpenMaterials = (sub: Subject) => {
    setMaterialsSubject(sub);
    ...
    <div className={`flex h-screen overflow-hidden ${theme === 'dark' ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <main className="flex-1 flex flex-col relative w-full overflow-hidden">
        <ChatWindow 
          ...
          theme={theme} 
          messages={chatSessions[activeSubject]}
